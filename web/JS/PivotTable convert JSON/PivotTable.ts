...
class PivotDate {
    ...
    public xAxisKeys: any = [];
    public xAxisKeysMap: any = {};

    public yAxisKeys: any = [];
    ...
    private _aggregator: any = function (argument) {
        return {
            data: [],
            put: function (record) {
                if (Array.isArray(record)) this.data = this.data.concat(record);
    ...
    // создание дерева массива
    private _processRecord(data: any): void {
        // ключи []
        ...
        const yAxisKeyFlat: string = yAxisKey.join(' ');
        ...
        if (xAxisKey.length > 0 && yAxisKey.length > 0) {
    ...
    // сортировка ключей
    private _sortXkeys() {
        this.xAxisKeys = this.xAxisKeys.sort(this._compareArr);
    }
    ...
    public getDataXY(xAxis: any, yAxis: any): any {
        ...
        if (flatXKey.length === 0 && flatYkey.length === 0) return null;
        else if (flatXKey.length === 0) return this.yAxisKeysMap[flatYkey];
        else if (flatYkey.length === 0) return this.xAxisKeysMap[flatXKey];
        else {
            const branch = this.tree[flatXKey];
            return branch[flatYkey] ? branch[flatYkey] : null;
        }

    }


    private _compareArr = (a, b, index) => {
        ...
        else return this._compareArr(a, b, ++i);
    }
...
const options: IOptions = {
    xAxis: ['Province', 'Gender'],
    yAxis: ['Name', 'Party'],
    measures: ['Age'],
}

const a = new PivotDate(data, options)
console.log(a)
let hasData = 0;
